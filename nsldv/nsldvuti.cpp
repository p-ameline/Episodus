...
long NVLdVTemps::lObjectCount = 0 ;

#ifndef _ENTERPRISE_DLL

long NVLdVPoint::lObjectCount = 0 ;
long NVLdVRect::lObjectCount = 0 ;

// --------------------------------------------------------------------------
// ------------------------- METHODES DE NVLdVPoint -------------------------
// --------------------------------------------------------------------------

NVLdVPoint::NVLdVPoint(NSLdvViewBase* pTheView)
{
	_tX.init() ;
	_dY = 0 ;
...
NVLdVPoint::NVLdVPoint(const NVLdVPoint& rv)
{
	_tX    = rv._tX ;
	_dY    = rv._dY ;
...
{
	_tX = _pView->getDateHeureInfDroit() ;
	//
	// Calcul de la différence en pixels sur l'axe des X (temps)
	//
	int iDelta = _pView->getTimeUnitFromPhysicalWidth(pPt->x - pOrigine->x) ;
	//
	// Conversion en AAAAMMJJhhmmss
	//
	switch (_pView->getXunit())
	{
		case pixAns :
			_tX.ajouteAns(iDelta) ;
			break ;
		case pixMois :
			_tX.ajouteMois(iDelta) ;
			break ;
		case pixSemaine :
			_tX.ajouteJours(iDelta * 7) ;
			break ;
		case pixJour :
			_tX.ajouteJours(iDelta) ;
			break ;
		case pixHeure :
			_tX.ajouteHeures(iDelta) ;
			break ;
		case pixMinute :
			_tX.ajouteMinutes(iDelta) ;
			break ;
		case pixSeconde :
			_tX.ajouteSecondes(iDelta) ;
			break ;
	}
	//
	// Calcul de la différence en pixels sur l'axe des Y
	//
	_dY = lWorkingAreaPosInfDroit + long(pOrigine->y - pPt->y) ;
}
...
NVLdVRect::NVLdVRect(NSLdvViewBase* pTheView)
{
  lTop    = 0 ;
  lBottom = 0 ;
...
NVLdVRect::NVLdVRect(NSLdvViewBase* pTheView, NVLdVTemps tDeb, NVLdVTemps tFin,
...
NVLdVRect::NVLdVRect(NSLdvViewBase* pTheView, NVLdVPoint* upLeft, NVLdVPoint* loRight)
{
  pView = pTheView ;
...
NVLdVRect::NVLdVRect(const NVLdVRect& rv)
{
  tLeft   = rv.tLeft ;
  tRight  = rv.tRight ;
  lTop    = rv.lTop ;
...
NVLdVTemps::NVLdVTemps(const NVLdVTemps& rv)
{
  _sTemps = rv._sTemps ;
...
NVLdVTemps::initFromDate(string sDate)
{
	...
	_sTemps = sDate + string(DATETIMESTRLEN - DATESTRLEN, '0') ;
...
string
NVLdVTemps::donneFormattedDateHeure(NSContexte* pContexte, string sLang) const
{
	string sTimeFormat = pContexte->getSuperviseur()->getText("0localInformation", "timeFormat") ;
  string formatDateHeure = getFormatedTime(_sTemps, sLang, sTimeFormat, "") ;
	return formatDateHeure ;
}

string
NVLdVTemps::donneFormattedDate(NSContexte* pContexte, string sLang) const
{
	string sDateFormat = pContexte->getSuperviseur()->getText("0localInformation", "dateFormat") ;
  string formatDate = getFormatedDate(string(_sTemps, 0, DATESTRLEN), sLang, sDateFormat) ;
	return formatDate ;
}

string
NVLdVTemps::donneFormattedHeure(NSContexte* pContexte, string sLang) const
{
	string sHourFormat = pContexte->getSuperviseur()->getText("0localInformation", "hourFormat") ;
  string formatHeure = getFormatedHour(string(_sTemps, DATESTRLEN, DATETIMESTRLEN - DATESTRLEN), sLang, sHourFormat) ;
	return formatHeure ;
}
...
int
NVLdVTemps::donneAns() const
{
	return donneDigit(_sTemps[0]) * 1000 +
         donneDigit(_sTemps[1]) * 100  +
         donneDigit(_sTemps[2]) * 10   +
         donneDigit(_sTemps[3]) ;
}

int
NVLdVTemps::donneMois() const
{
	return donneDigit(_sTemps[4]) * 10 + donneDigit(_sTemps[5]) ;
}

int
NVLdVTemps::donneJours() const
{
	return donneDigit(_sTemps[6]) * 10 + donneDigit(_sTemps[7]) ;
}

int
NVLdVTemps::donneHeures() const
{
	return donneDigit(_sTemps[8]) * 10 + donneDigit(_sTemps[9]) ;
}

int
NVLdVTemps::donneMinutes() const
{
	return donneDigit(_sTemps[10]) * 10 + donneDigit(_sTemps[11]) ;
}

int
NVLdVTemps::donneSecondes() const
{
	return donneDigit(_sTemps[12]) * 10 + donneDigit(_sTemps[13]) ;
}
...
void
NVLdVTemps::convert(ClassLib::TDate *td2Convert)
{
...
void
NVLdVTemps::convert(ClassLib::TTime tt2Convert)
{
...
string
NVLdVTemps::donneMonthName() const
{
try
{
...
{
	if ((0 == iDeltaT) || (string("") == sUnite) || ((NSContexte*) NULL == pCtx))
  	return ;
...
unsigned long
NVLdVTemps::yearsBetween(const NVLdVTemps& otherDate) const
{
...
unsigned long
NVLdVTemps::monthsBetween(const NVLdVTemps& otherDate) const
{
...
unsigned long
NVLdVTemps::daysBetween(const NVLdVTemps& otherDate) const
{
	// Par sécurité, on prend d'abord une copie normalisée
  //
	NVLdVTemps NormThis(*this) ;
...
unsigned long
NVLdVTemps::hoursBetween(const NVLdVTemps& otherDate) const
{
}

unsigned long
NVLdVTemps::minutesBetween(const NVLdVTemps& otherDate) const
{
}

unsigned long
NVLdVTemps::secondsBetween(const NVLdVTemps& otherDate) const
{
}
...
TimeArray::TimeArray(const TimeArray& rv)
          :TimeVector()
{
try
{
  if (false == rv.empty())
    for (TimeConstIter i = rv.begin() ; rv.end() != i ; i++)
...
try
{
  ...
  if (false == src.empty())
    for (TimeConstIter i = src.begin() ; src.end() != i ; i++)
    	push_back(new NVLdVTemps(*(*i))) ;
...
NSPixel::NSPixel(const NSPixel& rv)
{
  point = rv.point ;
  color = rv.color ;
...
PixelArray::PixelArray(const PixelArray& rv)
           :PixArray()
{
try
{
  ...
  if (false == rv.empty())
    for (pixelConstIter i = rv.begin() ; rv.end() != i ; i++)
...
try
{
  if (this == &src)
	...
	if (false == src.empty())
  	for (pixelConstIter i = src.begin() ; src.end() != i ; i++)
    	push_back(new NSPixel(*(*i))) ;
...
NSBitmap::NSBitmap(const NSBitmap& rv)
{
try
{
  pBitmap = (TDib*) 0 ;
...
{
  if (((TWindowDC*) NULL == pWinDC) || ((ClassLib::TRect*) NULL == pSrcRect))
    return false ;
...
try
{
  ...
  {
    delete pBitmap ;
    pBitmap = (OWL::TBitmap*) 0 ;
  }

  int iRectWidth  = pSrcRect->Width() ;
  int iRectHeight = pSrcRect->Height() ;
  ...
  //
  OWL::TBitmap* pBMap = new OWL::TBitmap(*pWinDC, iRectWidth, iRectHeight) ;
  ...
  //
  if (pMemDC->BitBlt(0, 0, iRectWidth, iRectHeight, *pWinDC, 0, 0))
  {
...
{
  erreur("Exception NSBitmap::ScreenCapture.", standardError, 0) ;
  return false ;
}
}
...
{
  erreur("Exception NSBitmap::Affiche.", standardError, 0) ;
  return false ;
}
}
...
NSOCRbloc::NSOCRbloc(const NSOCRbloc& rv)
{
...
OCRBlocsArray::OCRBlocsArray(const OCRBlocsArray& rv)
              :OCRArray()
{
try
{
  if (false == rv.empty())
    for (blocConstIter i = rv.begin() ; rv.end() != i ; i++)
...
try
{
	...
	if (false == src.empty())
  	for (blocConstIter i = src.begin() ; src.end() != i ; i++)
    	push_back(new NSOCRbloc(*(*i))) ;
