...
void writebmp(char *name,pix p,int vvv){ 	// see pcx.format.txt
  ...
  for(y=ny-1;y>=0;y--){
    if(((int)fwrite(pic+nx*y,1,nx,f1))!=nx)ERR("write");
    if(nx&3)
    if(((int)fwrite(rest,1,4-(nx&3),f1))!=4-(nx&3))ERR("write");  // must be mod4
  }
  fclose(f1);
}
