...
gocrList liblist;
gocrList modulefunclist[allModules];
gocr_Block empty;
...
struct _gocr_modulefunction {
	...
	union {
		void (*imageFilter) (gocr_Image * img, void *v);
...
int _gocr_initModule ( void ) {
	...
	/* init lists */
	gocrlist_init(&liblist);
	for (i = 0; i < allModules; i++) {
		gocrlist_init(&modulefunclist[i]);
	}
	...
	for (i = imageFilter; i < allModules; i++) {
		for_each_data(&modulefunclist[i]) {
			m = (struct _gocr_modulefunction *)
				gocrlist_get_current(&modulefunclist[i]);
			if (m) {
				if (m->name) {
					free(m->name);
					m->name = NULL;
		...
		gocrlist_free(&modulefunclist[i]);
	}

/*
	for_each_data(&liblist) {
		struct _gocr_lib *t =
				(struct _gocr_lib *)gocrlist_get_current(&liblist);
		lt_dlclose(t->handle);
		free(t);
	} end_for_each(&liblist);
	gocrlist_free(&liblist);
...
void gocr_moduleClose ( gocr_ModuleId id ) {
	...
	/* search module */
	for_each_data(&liblist) {
		l = (struct _gocr_lib *)gocrlist_get_current(&liblist);
		if (l->id == id)
			break;
	}
	end_for_each(&liblist);
	...
	for (t = 0; t < allModules; t++) {
		for_each_data(&modulefunclist[t]) {
			m = (struct _gocr_modulefunction *)
				gocrlist_get_current(&modulefunclist[t]);
			if (m->l == l) {
				if (m->name)
					free(m->name);
				gocrlist_del(&modulefunclist[t], m);
				free(m);
			}
		}
...
gocr_ModuleFunctionId gocr_functionInsertBefore ( char *functionname,
	...
	for_each_data(&liblist) {
		n.l = (struct _gocr_lib *)gocrlist_get_current(&liblist);
		if (n.l->id == mid) {
			flag++;
	...
	// and add to the list!
	if (func == -1) {
		if (gocrlist_app(&modulefunclist[finfo->type], m) == -1)
		...
		for_each_data(&modulefunclist[finfo->type]) {
			n = (struct _gocr_modulefunction *)
				gocrlist_get_current(&modulefunclist[finfo->type]);
			if (n->id == func)
				break;
		} end_for_each(&modulefunclist[finfo->type]);

		if (n == NULL)
			gocrlist_app(&modulefunclist[finfo->type], m);
		else
			gocrlist_ins(&modulefunclist[finfo->type], n, m);
	}
...
int gocr_runModuleType ( gocr_ModuleType t ) {
  ...
  for_each_data(&modulefunclist[t]) {
    m = (struct _gocr_modulefunction *)gocrlist_get_current(&modulefunclist[t]);
//    m->func(m->data);
  } end_for_each(&modulefunclist[t]);
}
#endif
...
int gocr_runAllModules ( void ) {
	...
	for_each_data(&modulefunclist[imageFilter]) {
		m = (struct _gocr_modulefunction *)
			gocrlist_get_current(&modulefunclist[imageFilter]);
	...
	for_each_data(&modulefunclist[blockFinder]) {
		m = (struct _gocr_modulefunction *)
			gocrlist_get_current(&modulefunclist[blockFinder]);
	...
	if (_data.no_block == GOCR_TRUE && gocrlist_total(&blocklist) == 0) {
		empty.x0 = empty.y0 = 0;
	...
	for_each_data(&blocklist) {
		currentblock = (gocr_Block *) gocrlist_get_current(&blocklist);
		for_each_data(&modulefunclist[charFinder]) {
			m = (struct _gocr_modulefunction *)
				gocrlist_get_current(&modulefunclist[charFinder]);
			_gocr_printf(3, FUNCTION, "module charFinder:%s\n",
				     m->name);
			if (m->func.charFinder((gocr_Block *)gocrlist_get_current(&blocklist), 
	...
	for_each_data(&blocklist) {
		...
		for_each_data(&currentblock->boxlist) {
			currentbox = (gocr_Box *) 
				gocrlist_get_current(&currentblock->boxlist);

			for_each_data(&modulefunclist[charRecognizer]) {
				m = (struct _gocr_modulefunction *)
					gocrlist_get_current(&modulefunclist
							 [charRecognizer]);
				_gocr_printf(3, FUNCTION,
	...
	for_each_data(&blocklist) {
		...
		for_each_data(&currentblock->boxlist) {
			for_each_data(&modulefunclist[contextCorrection]) {
				...
				if (m->
				    func((gocr_Box *)
					 gocrlist_get_current(&currentblock->
							  boxlist),
					 m->data)) break;
			}
	...
	for_each_data(&blocklist) {
		_gocr_blockTextFill((gocr_Block *) gocrlist_get_current(&blocklist));
	} end_for_each(&blocklist);
	
	/* output */
	/* first store the list in an array */
	blist = (gocr_Block **)malloc((gocrlist_total(&blocklist)+1) *
			sizeof(gocr_Block *));
	if (!blist) {
	...
	for_each_data(&blocklist) {
		blist[i++] = (gocr_Block *)gocrlist_get_current(&blocklist);
	} end_for_each(&blocklist);
	blist[i] = NULL;
	...
	for_each_data(&modulefunclist[outputFormatter]) {
		_gocr_printf(3, FUNCTION, "module outputFormatter:%s\n",
			     m->name);
		m = (struct _gocr_modulefunction *)
			gocrlist_get_current(&modulefunclist[outputFormatter]);
