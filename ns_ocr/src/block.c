...
gocr_blockId gocr_blockAdd ( gocr_Block * b ) {
	...
	if (_data.block_overlap == 0) {
		...
		for_each_data(&blocklist) {
			...
			t = (gocr_Block *) gocrlist_get_current(&blocklist);
...
//			gocr_isblock(currentimage, j, i) = 1;
            NSgocr_isblock(currentimage, j, i, 1);

    if ((pTest3 = (char *) malloc(10)) != NULL)
        strcpy(pTest3, "Hello");


	b->text = NULL;
	gocrlist_init(&b->boxlist);

	/* append to block list */
	if (gocrlist_app(&blocklist, b)) {
...
void NSgocr_isblock(gocr_Image *image, int x, int y, char value)
{
    char*       pTest1;
    char*       pTest2;
    size_t      iPixSize;
    gocr_Pixel  *pPix;
    gocr_PixelBW    *pPixBW;
    gocr_PixelGray  *pPixGr;
    gocr_PixelColor *pPicCl;

    if ((pTest1 = (char *) malloc(10)) != NULL)
        strcpy(pTest1, "Hello");

    iPixSize = _gocr_imagePixelSize(image);
    //pPix = (gocr_Pixel *)((image)->data.pix[y]+(x)*iPixSize);
    pPix = (gocr_Pixel *)((image)->data.pix[y]);
    ...
    switch (image->type) {
	    ...
	    case GOCR_OTHER:
		    return;
	    case GOCR_BW:
            pPixBW = (gocr_PixelBW *)pPix;
            pPixBW += x * iPixSize;
            pPixBW->isblock = value;
		    break;
	    case GOCR_GRAY:
            pPixGr = (gocr_PixelGray *)((image)->data.pix[y]+(x)*iPixSize);
            pPixGr->isblock = value;
		    break;
	    case GOCR_COLOR:
            pPicCl = (gocr_PixelColor *)((image)->data.pix[y]+(x)*iPixSize);
            pPicCl->isblock = value;
		    break;
    }
    if ((pTest2 = (char *) malloc(10)) != NULL)
        strcpy(pTest2, "Hello");

 //((gocr_Pixel *)((image)->data.pix[y]+(x)*_gocr_imagePixelSize(image)))->isblock = value;
}
...
void _gocr_blockInfoFill ( gocr_Block *b ) {
	...
	for_each_data(&b->boxlist) {
		box = (gocr_Box *) gocrlist_get_current(&b->boxlist);
	...
	b->boxinfo.av_height /= (float)gocrlist_total(&b->boxlist);
	b->boxinfo.av_width /= (float)gocrlist_total(&b->boxlist);
