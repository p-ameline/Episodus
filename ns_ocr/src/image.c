...
static gocrList garbage;

int _gocr_initImage ( void ) {
	gocrlist_init(&garbage);
	return 0;
}
...
/**
  ...
  \param newImg A pointer to the new image. May be NULL.
...
int _gocr_imageSharedCopy ( gocr_Image *orig, int x0, int y0, int x1, int y1,
...
#ifdef notnotnot /* this would require gocr_Image **new. */
	if (newImg == NULL) {
		newImg = (gocr_Image *) malloc(sizeof(gocr_Image));
		if (newImg == NULL) {
			_gocr_printf(1, FUNCTION, "NULL malloc\n");
			return -1;
		}
	...
	newImg->filename = NULL;
	newImg->width = x1 - x0 + 1;
	newImg->height = y1 - y0 + 1;
	newImg->type = orig->type;
	switch (newImg->type) {
	  case GOCR_NONE:
	  case GOCR_OTHER:
	  ...
	  case GOCR_BW:
        pixSize = sizeof(gocr_PixelBW *);
		newImg->data.pix = malloc(newImg->height * pixSize);
		break;
	  case GOCR_GRAY:
		newImg->data.pix = malloc(newImg->height * sizeof(gocr_PixelGray *));
		break;
	  case GOCR_COLOR:
		newImg->data.pix = malloc(newImg->height * sizeof(gocr_PixelColor *));
		break;
	}
	if (newImg->data.pix == NULL) {
		_gocr_printf(1, FUNCTION, "NULL malloc 2\n");
		return -1;
	}
	if (orig->mask) {
		newImg->mask = malloc(((newImg->height * newImg->width) >> 3) + 1);
		if (newImg->mask == NULL) {
			_gocr_printf(1, FUNCTION, "NULL malloc 3\n");
			return -1;
		}
		memcpy(newImg->mask, orig->mask, ((newImg->height*newImg->width)>>3) + 1);
	}
	else 
		newImg->mask = NULL;
		
	for (i = 0; i < newImg->height; i++)
		newImg->data.pix[i] = orig->data.pix[i + y0] + 
				x0 * _gocr_imagePixelSize(newImg);

	if (orig->parent) {
		newImg->parent = orig->parent;
		newImg->sons = 0;
	}
	else {
		newImg->parent = orig;
...
/**
  \brief fill new image, copying the data to a new memory location
...
int _gocr_imageCopy ( gocr_Image *orig, int x0, int y0, int x1, int y1,
	...
	for (i = 0; i < newImg->height; i++)
		memcpy(newImg->data.pix[i], orig->data.pix[i + y0] +
			       x0 * _gocr_imagePixelSize(newImg), newImg->width);
	return 0;
}
...
void gocr_imageClose ( void ) {
	...
	for_each_data(&blocklist) {
		block = (gocr_Block *)gocrlist_get_current(&blocklist);
		for_each_data(&block->boxlist) {
			box = (gocr_Box *)gocrlist_get_current(&block->boxlist);
			gocr_imageFree(box->image); 
			for_each_data(&box->possible) {
				gocr_Char *c = (gocr_Char *)gocrlist_get_current(&box->possible);
				free(c);
			} end_for_each(&box->possible);
			gocrlist_free(&box->possible);
			free(box);
		} end_for_each(&block->boxlist);
		gocrlist_free(&block->boxlist);
		gocr_imageFree(block->image);
/*		free(block); */
	} end_for_each(&blocklist);
	gocrlist_free(&blocklist);
...
void gocr_imageFree ( gocr_Image *image ) {
	...
	if (image->sons == 0) { /* no children, free it */
		if (image->parent) { /* is shared */
			...
			if (image->parent->sons == 0) {
				...
				for_each_data(&garbage) {
					par = (gocr_Image *)gocrlist_get_current(&garbage);
					if (par == image->parent)
						break;
				} end_for_each(&garbage);
				if (par == image->parent) {
					gocrlist_del(&garbage, par);
					gocr_imageFree(par);
				}
			}
			free(image->data.pix);
		...
		if (!newImg) {
			...
			memcpy(newImg, image, sizeof(gocr_Image));
			free(image);
		}
		gocrlist_app(&garbage, newImg);
	}
	return;
}
