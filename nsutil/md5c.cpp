...
void
MD5_encrypt(unsigned char szPassEncrypted[16], char *szPass)
{
  MD5_CTX       context ;
  unsigned int  len = strlen(szPass) ;

  MD5Init(&context) ;
  MD5Update(&context, szPass, len) ;
  MD5Final(szPassEncrypted, &context) ;
}


string
MD5_encrypt(string sPass)
{
  unsigned char szPassEncrypted[16] ;
  string        sPassEncrypted      = "" ;
  
  MD5_encrypt(szPassEncrypted, (char *)(sPass.c_str())) ;

  for (int i = 0 ; i < 16 ; i++)
  {
    char  szTemp[3] ;
    sprintf(szTemp, "%02x", szPassEncrypted[i]) ;
    sPassEncrypted += szTemp ;
  }

  return (sPassEncrypted) ;
}
