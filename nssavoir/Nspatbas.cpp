...
//---------------------------------------------------------------------------
int
NSPathoBaseData::operator==(const NSPathoBaseData& o) const
{
  for (int i = 0 ; i < 4 ; i++)
...
bool
NSPathoBaseData::sameContent(const NSPathoBaseData& o) const
{
	//
...
bool
NSPathoBaseData::sameContentButInterest(const NSPathoBaseData& o) const
{
	//
  // Le champ "codeLocalisation" est exclu de la comparaison
  //
	// Le champ "visible", qui n'est pas à proprement parler un champ
	// de données est exclu de la comparaison, idem pour "intérêt"
	//
	return ((_sType             == o._sType) &&
          (_sLexique          == o._sLexique) &&
          (_sComplement       == o._sComplement) &&
