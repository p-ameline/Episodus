...
DBIResult V2_3Patient::importe()
{
  	...
  	if (lastError != DBIERR_NONE)
   {
      	 ...
      	 (strcmp(pAdrImport->pDonnees->adresse2, "                         ") != 0))
		{
      	if (pAdrImport->LastCode(&sCode,tCodeAdresse))
      	{
            ...
            lastError = pPatImport->appendRecord();
      		if (lastError != DBIERR_NONE)
      		{
      			erreur("Erreur à l'ajout de la fiche patient", 0, lastError);
         		fclose(infile);
         		lastError = pPatImport->close();
         		delete pPatImport;
         		lastError = pPcoImport->close();
      ...
      for (j = 0; (j < 6) && (strcmp(V2medic[j], "   ") != 0); j++)
      {
         ...
         if (lastError != DBIERR_NONE)
         {
         	erreur("Erreur à l'ajout de la fiche patcor", 0, lastError);
            fclose(infile);
            lastError = pPatImport->close();
            delete pPatImport;
            lastError = pPcoImport->close();
      ...
      if (strcmp(V2medinom, "                         ") != 0)
      {
         ...
         if (lastError != DBIERR_NONE)
         {
					erreur("Erreur à l'ajout de la fiche adresse", 0, lastError);
					fclose(infile);
					return(lastError);
				}
				...
				if (lastError != DBIERR_NONE)
				{
					erreur("Erreur à l'ajout de la fiche patcor", 1, lastError);
					fclose(infile);
					return(lastError);
				}
      }
