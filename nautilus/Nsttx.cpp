...
try
{
  ...
  if (_pApplicationProxy && _pApplicationProxy->IsBound())
	...
	if (_pDocTtx->_pDocInfo)  // si le document est référencé	{
    ...
    for (int j = 0; j < iTailleBuf; j++)
    {
    	switch (buffer[j])
      {
          ...
          break ;        }        case 28 :
        {
          ...
          if (string("") != sText)
          {
            // Replace "ECHOGRAPHIE" by "ÉCHOGRAPHIE ABDOMINALE TRANSCUTANÉE"
            //
            if (false == bTitleUpdated)
            {
              ...
              if (NPOS != posit)
              {
                sText.replace(posit, 11, "ÉCHOGRAPHIE ABDOMINALE TRANSCUTANÉE") ;
                bTitleUpdated = true ;
              }
            }
