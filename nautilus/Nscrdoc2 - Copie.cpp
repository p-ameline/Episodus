...
try
{
  ...
  string ps = string("Export using dll ") + sExportDll ;
  pSuper->trace(&ps, 1) ;

  TModule* pDCModule = new TModule(sExportDll.c_str(), TRUE) ;
  if ((TModule*) NULL == pDCModule)
  {
    string sErrorText = pContexte->getSuperviseur()->getText("modulesManagement", "cannotLoadDll") + string(" ") + sExportDll ;
  ...
  {
    string sErrorText = pContexte->getSuperviseur()->getText("modulesManagement", "cannotAccessMethodInDll") + string(" ") + sExportDll + string(" -> needExportFile") ;
    pSuper->trace(&sErrorText, 1) ;
    erreur(sErrorText.c_str(), standardError, -1, pContexte->GetMainWindow()->GetHandle()) ;
    pSuper->afficheStatusMessage("") ;
  }
  ...
  if (NULL == pAdresseFct)
  {
    string sErrorText = pContexte->getSuperviseur()->getText("modulesManagement", "cannotAccessMethodInDll") + string(" ") + sExportDll + string(" -> exporter") ;
    pSuper->trace(&sErrorText, 1) ;
    erreur(sErrorText.c_str(), standardError, -1, pContexte->GetMainWindow()->GetHandle()) ;
  ...
  ps = string("Leaving export using dll ") + sExportDll ;
  pSuper->trace(&ps, 1) ;
...
{
	erreur("Exception NSCRDocument::export", standardError, 0) ;
  return false ;
}
}
