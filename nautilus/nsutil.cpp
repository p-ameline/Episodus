...
//---------------------
  ...
  lpNewProc = MakeProcInstance((FARPROC)lpChangeProc, hInst);
  lpOldProc = (WNDPROC) SetWindowLong(hWndEdit, GWL_WNDPROC, (long)lpNewProc);
}
//---------------------------------------------------------------------------
...
//--------------------
  hWndEdit = GetDlgItem(hWnd, id_Edit);
  lpNewProc = (FARPROC)SetWindowLong(hWndEdit, GWL_WNDPROC, (long)lpPrevProc);
...
//-------------------------------------------------
  switch(wMessage) {
    case WM_CHAR : if (((wParam < '0') || (wParam > '9')) && wParam != 0x08)
		      MessageBeep(0);
		   else
		      bTraite = FALSE;
		   break;
