...
try
{
	...
	if (iGravite > 0)
  {
    ...
    pContexte->getPatient()->pGraphPerson->setTree(pPt, "") ;
		...
		{
			erreur("Erreur à l'inscription de la gravité et du risque : le document Index de santé de ce patient semble endommagé.", standardError, 0) ;
			return false ;
		}
		iter = pPt->ChercherItem("0PRO11") ;
		if ((iter == NULL) || (iter == pPt->end()))
		{
			erreur("Erreur à l'inscription de la gravité et du risque : le document Index de santé de ce patient ne possède pas de chapitre Préoccupations de santé.", standardError, 0) ;
			return false ;
		}
		int iConcernCol = (*iter)->getColonne() ;
		iter++ ;
