  ...
  public:
	  ...
	  CICalendarAttribute& operator=(const CICalendarAttribute& copy)
		  { if (this != &copy) _copy(copy); return *this; }
	        ...
	        cdstring& GetName()       { return mName ; }
    const cdstring& GetName() const { return mName ; }
	  void  SetName(const cdstring& name) { mName = name ; }

	  const cdstring& GetFirstValue() const { return mValues.front() ; }
	        cdstrvect& GetValues()       { return mValues ; }
	  const cdstrvect& GetValues() const { return mValues ; }
	  void AddValue(const cdstring& value) { mValues.push_back(value) ; }
...
typedef multimap<cdstring, CICalendarAttribute> CICalendarAttributeMap;
