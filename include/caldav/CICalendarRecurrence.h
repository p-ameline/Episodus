...
{
public:
	typedef pair<int32_t, CICalendarDateTime::EDayOfWeek>	CWeekDayNum;
	...
	const vector<int32_t>& GetByMonth() const
		{ return mByMonth; }
	void SetByMonth(const vector<int32_t>& by)
		{ mByMonth = by; }
		
	const vector<int32_t>& GetByMonthDay() const
		{ return mByMonthDay; }
	void SetByMonthDay(const vector<int32_t>& by)
		{ mByMonthDay = by; }
		
	const vector<CWeekDayNum>& GetByDay() const
		{ return mByDay; }
	void SetByDay(const vector<CWeekDayNum>& by)
		{ mByDay = by; }
		
	const vector<int32_t>& GetBySetPos() const
		{ return mBySetPos; }
	void SetBySetPos(const vector<int32_t>& by)
		{ mBySetPos = by; }
...
protected:
	...
	int32_t						mInterval;
	vector<int32_t>				mBySeconds;
	vector<int32_t>				mByMinutes;
	vector<int32_t>				mByHours;
	vector<CWeekDayNum>			mByDay;
	vector<int32_t>				mByMonthDay;
	vector<int32_t>				mByYearDay;
	vector<int32_t>				mByWeekNo;
	vector<int32_t>				mByMonth;
	vector<int32_t>				mBySetPos;
...
private:
	typedef map<cdstring, ERecurrence_FREQ>	CFreqMap;
	static CFreqMap		sFreqMap;

	void _init_CICalendarRecurrence();
	void _copy_CICalendarRecurrence(const CICalendarRecurrence& copy);

	bool Equals(const vector<int32_t>& items1, const vector<int32_t>& items2) const;
	bool Equals(const vector<CWeekDayNum>& items1, const vector<CWeekDayNum>& items2) const;

	void ParseList(const char* txt, vector<int32_t>& list);
	void ParseList(const char* txt, vector<CWeekDayNum>& list);
...
typedef vector<CICalendarRecurrence>	CICalendarRecurrenceList;
