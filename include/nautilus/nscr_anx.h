...
class NSLigne
{
  public:
    ...
    UINT 	    iNumPhrase;			// Numéro de la phrase de rattachement

    string	    Texte; 				// string qui contient le contenu de la ligne
    UINT		numPage; 			// Numéro de la page qui contient la ligne
    UINT		numLigne; 			// Numéro de la ligne dans le paragraphe
                    // et indicateur de dernière ligne
    bool		DerniereLigne; 	// (utile pour les paragraphes justifiés)

    NS_CLASSLIB::TRect  	Boite, BoitePrnt; // Boite dans laquelle est contenue la ligne
    NS_CLASSLIB::TColor 	CouleurTexte;

    UINT		numParagraphe; 	// Numéro du style de paragraphe utilisé
                    // pour cette ligne
    ...
    void Reinitialiser() ;  // Remet entièrement à zéro
