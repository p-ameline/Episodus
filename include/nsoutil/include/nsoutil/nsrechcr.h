...
class _NSOUTILCLASSE NSCritReqPatient
{
    ...
    char sexe[PAT_SEXE_LEN + 1];
    char dateN1[PAT_NAISSANCE_LEN + 1];
    char dateN2[PAT_NAISSANCE_LEN + 1];
    char sitfam[PAT_SITFAM_LEN + 1];
...
class _NSOUTILCLASSE NSRequestResult
{
    ...
    void   copyValues(NSRequestResult* pSrc) ;
...
class _NSOUTILCLASSE NSRequeteDialog : public NSUtilDialog
{
    ...
    NSUtilEdit2*             pNomReq ;
    NSRequeteWindow*         pListeChem ;
    NSBooleanRequestsWindow* pRequestsList ;

    NSUtilEdit2*        pEditReqLabel ;    // Boite de saisie du nom de la phrase logique
    NSUtilEdit2*        pEditReq ;         // Boite de saisie de la phrase logique

    OWL::TGroupBox*		  pMode ;
    OWL::TRadioButton*	pModePat ;
    OWL::TRadioButton*  pModeDoc ;
    string              sFileName ;
    bool                bReqModeDoc ;
    bool                bReqEnCours ;
    int                 nbPatTotal ;
    ...
    int                 nbDocCritDoc ;
    int                 nbDocResult ;
    NSDocumentHisto     Document ;           // Document en cours pour la recherche
    NSPatInfo           Patient ;            // Patient en cours pour la recherche

    OWL::TCheckBox*		  pCritPatBox ;
    OWL::TCheckBox*    	pCritDocBox ;
    bool                bCritPat ;
    bool                bCritDoc ;
    NSCritReqPatient*   pCritPat ;
    NSCritReqDocum*     pCritDoc ;
    bool                bCherchePatient ;
    bool                bChercheDocument ;

    NSLanceReqDialog*         pLanceur ;
    NSPersonsAttributesArray* pPersonList ;
    int                       iPersonListSize ;
    int                       iCurrentPatient ;
    NSDocHistoArray           VectDocument ;
...
class NSEditReqDialog : public NSUtilDialog
{
    ...
    void TrouveLibelleValeur(string sCode, string& sLibelle, bool bPere) ;
