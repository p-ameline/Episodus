...
#endif
{
	public:
		...
		ParseSOAP(NSContexte* pCtx, const string sClassifier, const string sChapter) ;
		~ParseSOAP();
	...
	protected:
		...
		string              _sClassifier ;         // Classification
    string              _sChapter ;            // Chapter (S, A, P)

		string              *_psSOAPChemin ;
		string              *_psSOAP2Parse ;
		string              *_psResultParsing ;
...
//        void                ReplaceJoker(string sBorn, string s2Replace);
    ...
    bool         doesContain(const string sCode) const ;

    void         reorder() ;

    ElemSet&     operator=(const ElemSet &src) ;
    bool         operator==(const ElemSet &src) ;
...
typedef ElemSetVector::const_iterator ElemSetConstIterator ;
...
#endif
{
  ...
  public:
    ...
    ElemSetArray(const ElemSetArray& rv) ;
...
typedef ParseCategoryVector::const_iterator ParseCategoryConstIterator ;
...
#endif
{
  ...
  public:
    ...
    TermsParser(NSContexte* pCtx, string sClassif, string sChapter, bool bUse3bt = true) ;
    ~TermsParser() ;
    ...
    string       getsObligatoire() { return _pObligatoire->toString() ; }
    string       getsPossible()    { return _pPossible->toString() ; }
    string       getsInterdit()    { return _pInterdit->toString() ; }
  ...
  protected:
    ...
    ElemSetArray *_pObligatoire ;
    ElemSetArray *_pPossible ;
    ElemSetArray *_pInterdit ;
...
#endif
{
  ...
  public:
    ...
    ParseElem(string sElem) ;
    ParseElem(string sElem, int nbTerms, int Pos) ;
    ParseElem(const ParseElem& rv) ;
    ~ParseElem() { lObjectCount-- ; } ;

    ParseElem& operator=(const ParseElem &src) ;

    bool    getNotInFirst()            { return _bNotInFirst ; }
    void    setNotInFirst(bool bValue) { _bNotInFirst = bValue ; }

    string  getContent()               { return _sContent ; }

    int     getNbOfTerms()             { return _nbOfTerms ; }
    void    setNbOfTerms(int i)        { _nbOfTerms = i ; }

    int     getPos()                   { return _iPos ; }
    void    setPos(int Pos)            { _iPos = Pos ; }
...
typedef ParseElemVector::const_iterator ParseElemConstIter ;
...
#endif
{
  private:
    ...
    ParseElemArray(const ParseElemArray& rv) ;
		~ParseElemArray() ;
...
#endif
{
  private:
	...
	public:
		...
		ClassifElem(string s2Code, string s2Libelle) ;
    ClassifElem(const ClassifElem& rv) ;
...
typedef ClassifElemVector::const_iterator ClassifElemConstIter ;
...
//        d'ou array de NSepiClassifInfo
  ...
  private:
    ...
    ClassifElemArray *_pChap ;
    ClassifElemArray *_pLibelle ;
} ;
