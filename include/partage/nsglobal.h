...
class NSSuperRoot
{
  protected:
    ...
    static long lSRObjectCount ;

  public:

    NSSuper* _pSuper ;

		NSSuperRoot(NSSuper* pSuper) : _pSuper(pSuper) { lSRObjectCount++ ; }
    virtual ~NSSuperRoot() { lSRObjectCount-- ; }

    NSSuper* getSuper() { return _pSuper ; }

    static long getSRNbInstance()  { return lSRObjectCount ; }
    static void initSRNbInstance() { lSRObjectCount = 0 ; }
};
...
class NSRoot
{
  protected:
    ...
    static long lRootObjectCount ;

  public:

		NSContexte* pContexte ;
		NSRoot(NSContexte* pCtx) : pContexte(pCtx) { lRootObjectCount++ ; }
    virtual ~NSRoot() { lRootObjectCount-- ; }

    static long getRootNbInstance()  { return lRootObjectCount ; }
    static void initRootNbInstance() { lRootObjectCount = 0 ; }
};

long NSSuperRoot::lSRObjectCount = 0 ;
long NSRoot::lRootObjectCount = 0 ;
