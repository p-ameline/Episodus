...
#endif
{
  private:
    ...
    NSPatPathoArray* getMetaPointer() { return &_Meta ; }
...
#endif
{
    ...
    int           hasSameContent(const NSSejourData& o) const ;
...
#endif
{
  private:
	...
	public:
		...
		NSSejourInfo() ;
		NSSejourInfo(const NSSejourInfo& rv) ;
		~NSSejourInfo() ;

		void donneIntitule(char *intitule) ;

		NSSejourInfo& operator=(const NSSejourInfo& src) ;
		int           operator==(const NSSejourInfo& o) const ;
    int           hasSameContent(const NSSejourInfo& o) const ;
    ...
    bool   initFromPatPatho(NSPatPathoArray *pPPT) ;
    bool   initFromCapture(NSCaptureArray* pCapt) ;
    bool   updateFromCapture(NSCaptureArray* pCapt) ;
    bool   buildPatPatho(NSPatPathoArray *pPPT) ;
    bool   updatePatPatho(NSPatPathoArray *pPPT) ;
...
#endif
{
  public :
    ...
    // Variables de stockage des valeurs
    //
    char _chemise[CDO_CHEMISE_LEN + 1] ;
    char _ordre[CDO_ORDRE_LEN + 1] ;
    char _document[CDO_DOCUMENT_LEN + 1] ;

    NSChemDocData() ;
    NSChemDocData(const NSChemDocData& rv) ;

    NSChemDocData& operator=(const NSChemDocData& src) ;
...
#endif
{
  ...
  protected:
    ...
    NSChemDocData _Donnees ;

  public :

    NSChemDocInfo() ;
    NSChemDocInfo(const NSChemDocInfo& rv) ;
    ~NSChemDocInfo() ;

    NSChemDocInfo& operator=(const NSChemDocInfo& src) ;
...
#endif
{
  ...
  public :
    ...
    NSDocStatus() ;
    NSDocStatus(const NSDocStatus& rv) ;
    ~NSDocStatus() ;

    NSDocStatus& operator=(const NSDocStatus& src) ;
