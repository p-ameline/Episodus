...
# define PREP4LOC			    "L"   // préposition pour la loc, ex "angioplastie de l'artère" vs "ponction au rein droit"
...
#endif
{
  public:
    ...
    NSPathologData() ;
    NSPathologData(const NSPathologData& rv) ;
    ~NSPathologData() ;
...
#endif
{
  public:
    ...
    NSPatholog(const NSPatholog& rv) ;
    virtual ~NSPatholog() ;
    ...
    bool					trouvePathologData(const string *pCode, NSPathologData *pData, bool afficheErreur = true) ;
    DBIResult			trouveCode(const string *pCode, DBISearchCond searchMode = keySEARCHEQ, DBILockType Blocage = dbiWRITELOCK, bool bErrMessage = true) ;
    DBIResult			trouveLibelle(const string *pLibelle, DBISearchCond searchMode = keySEARCHEQ, DBILockType Blocage = dbiWRITELOCK, bool bErrMessage = true) ;
#else
    bool					trouvePathologData(string sLang, const string *pCode, NSPathologData *pData, bool afficheErreur = true) ;
    bool          trouveCode(string sLang, const string *pCode, bool bVerbose = true) ;
    bool          trouveLibelle(string sLang, const string *pLibelle) ;
    bool          trouveLibelleApproche(string sLang, const string *pLibelle) ;
#endif
...
#endif
{
  public:
    ...
    NSPathologInfo(const NSPathologInfo& rv) ;
    ~NSPathologInfo() ;
...
#endif
{
  public:
    ...
    NSSavoirData() ;
    NSSavoirData(const NSSavoirData& rv) ;
    ~NSSavoirData() ;

    NSSavoirData& operator=(const NSSavoirData& src) ;
...
#endif
{
  public:
    ...
    // Objet qui contient les données
    NSSavoirData _Donnees ;

    NSSavoir(NSSuper* pSuper) ;
    NSSavoir(const NSSavoir& rv) ;
    ~NSSavoir() ;
    ...
    string getCode()      { return _Donnees.getCode() ; }
    string getQualified() { return _Donnees.getQualified() ; }
    string getLink()      { return _Donnees.getLink() ; }
    string getQualifier() { return _Donnees.getQualifier() ; }
    string getLevel()     { return _Donnees.getLevel() ; }
    string getClass()     { return _Donnees.getClass() ; }
    string getScenario()  { return _Donnees.getScenario() ; }
} ;
...
#endif
{
  public:
    ...
    NSSavoirInfo() ;
    NSSavoirInfo(const NSSavoir *) ;
    NSSavoirInfo(const NSSavoirInfo& rv) ;
    ~NSSavoirInfo() ;
