...
class _NSBBCLASSE BBFilsItem : public NSRoot
{
  protected:
    ...
    BBItem*			    _pPere ;
    BBItemData* 	  _pDonnees ;		// Données de la fiche GUIDES
...
class _NSBBCLASSE BBItem : public NSRoot
{
 ...
 public:
  ...
  BBFilsItem            *pBBFilsPere ;		        // Pointeur sur le BBFilsItem créateur de ce BBItem
  NSPatPathoArray       *pPPTEnCours ;		        // Pointeur sur la PatPatho attribuée par le pBBFilsPere
  ...
  bool                  bCreateParseur;             // booléen pour le cas où on transmet le parseur

  int				            iFilsActif ;	 		        // Numéro du BBFilsItem qui a la main

  bool                  bActif ;     	 	          // Indicateur d'activation d'un contrôle

  bool                  bMiseAJour ;	 		        // Faut-il tenir compte des modifs ?
  int                   iRetourDlg ;	 		        // Code de retour de la boite de dialogue
  int                   iStatusDlg ;                    // Code pour les réinitialisations d'archetypes

  int                   ID ;         		          // ID de l'élément d'interface qui correspond au fils
                                                  // de l'objet BigBrother en cours

  bool			            bConclusion ;             // savoir si le BBItem sert à la conclusion
  bool			            ConclusionAutomatique ;   // pré-existence de conclusion automatique
  string			          sPositionConclusion ;     // Avant A La place ou Après conclusion
  bool			            bCacher ;				          // cacher la NSdialog lancée par ce BBItem
