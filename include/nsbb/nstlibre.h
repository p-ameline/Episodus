...
class NSTexteLibreData
{
  public :
    ...
    NSTexteLibreData(const NSTexteLibreData& rv) ;
    NSTexteLibreData() { metAZero() ; }
    ~NSTexteLibreData() ;

    NSTexteLibreData& operator=(const NSTexteLibreData& src) ;
...
class NSTlibreInfo
{
    ...
    // Objet qui contient les donnees
		...
		NSTlibreInfo(const NSTlibreInfo& rv) ;
    ~NSTlibreInfo() ;

		NSTlibreInfo& operator=(const NSTlibreInfo& src) ;
