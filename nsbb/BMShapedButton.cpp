...
//    void __fastcall SetGlyphMask ( Graphics::TBitmap *Value );
    ...
    void __fastcall DrawButtonText ( TCanvas *Canvas, const AnsiString Caption,
    ...
    //  const TPoint Offset, bool DrawGlyph, bool DrawCaption, const AnsiString Caption,
...
public:
    ...
    //  bool DrawGlyph, bool DrawCaption, AnsiString const Caption, bool WordWrap,
...
//            Inc (Integer(Pixel), SizeOf(Longint));  { proceed to the next pixel }
          ...
          (int)MonoPixel = ( int )StartMonoPixel + MonoScanLineSize;
          }
        }
...
//---------------------------------------------------------------------------
void __fastcall OffsetTextOut ( TCanvas *Canvas, int XTextOffset, int YTextOffset, TRect TextBounds, const AnsiString Caption, UINT Format )
{
...
//---------------------------------------------------------------------------
void __fastcall DrawTextShadow ( TCanvas *Canvas, int ShadowOffset, const AnsiString Caption, TRect TextBounds, UINT Format, bool Extended )
{
  for ( int i = 0; ( ShadowOffset > 0 ) ? i <= ShadowOffset : i >= ShadowOffset ; ( ShadowOffset > 0 ) ? i ++ : i -- )
    {
    Canvas->CopyMode = cmSrcAnd;
...
//---------------------------------------------------------------------------
void __fastcall TButtonGlyph::DrawButtonText ( TCanvas *Canvas, const AnsiString Caption,
      TRect TextBounds, bool WordWrap, TAlignment Alignment, TBMButtonState State, int FReliefSize, TColor FHotTrackColor )
{
...
// End ED
{
  ...
  if ( Margin == -1 )
    ...
    if ( Layout == blGlyphCentre )
      {
      Margin = (ClientSize.x -
      (max(TextSize.x,GlyphSize.x) + ArrowSize.x + 2 * FReliefSize)) / 2;
      }
    else {
    // End ED
    ...
    //---> Begin ED
    //if ( Spacing == -1 )
    if ( Spacing == -1 && Layout != blGlyphCentre)
    // End ED
      {
  ...
  switch ( Layout )
    ...
    case blGlyphCentre:
    	{
      if (GlyphSize.x > TextSize.x)
        {
        ArrowPos.x = GlyphPos.x + GlyphSize.x;
        GlyphPos.x = Margin;
        TextPos.x = Margin + (GlyphSize.x - TextSize.x) / 2 + Spacing + FReliefSize;
        }
      else
        {
        ArrowPos.x = TextPos.x + TextSize.x;
        GlyphPos.x = Margin + (TextSize.x - GlyphSize.x) / 2;
        TextPos.x = Margin + Spacing + FReliefSize;
        }
      if (GlyphSize.y > TextSize.y)
        {
        GlyphPos.y = (ClientSize.y - (GlyphSize.y + 2 * FReliefSize)) / 2;
        TextPos.y = GlyphPos.y + (GlyphSize.y - TextSize.y) / 2 + FReliefSize;
        }
      else
        {
        TextPos.y = (ClientSize.y - (TextSize.y + 2 * FReliefSize)) / 2;
        GlyphPos.y = TextPos.y + (TextSize.y - GlyphSize.y) / 2;
        TextPos.y += FReliefSize;
        }
      }
    // End ED
...
{
  TR ( (AnsiString)"TBMShapedButton::IsHelpContextStored" + (AnsiString)(long)ActionLink );
  if ( ActionLink )
